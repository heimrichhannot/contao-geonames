...
class tl_geonames_postal extends Backend
{
	...
	public function checkPermission()
	{
		...
		// Set root IDs
		if (!is_array($this->User->areasoflaw) || empty($this->User->areasoflaw)) {
			$root = array(0);
		} else {
			$root = $this->User->dav;
		}

		$GLOBALS['TL_DCA']['tl_geonames_postal']['list']['sorting']['root'] = $root;

		// Check permissions to add item
		if (!$this->User->hasAccess('create', 'areasoflawp')) {
			$GLOBALS['TL_DCA']['tl_geonames_postal']['config']['closed'] = true;
		}
		...
		switch (Input::get('act')) {
			...
			case 'edit':
				...
				if (!in_array(Input::get('id'), $root)) {
					...
					if (is_array($arrNew['tl_geonames_postal']) && in_array(Input::get('id'), $arrNew['tl_geonames_postal'])) {
						// Add permissions on user level
						if ($this->User->inherit == 'custom' || !$this->User->groups[0]) {
							$objUser = $this->Database->prepare("SELECT geonames, geonamep FROM tl_user WHERE id=?")
								->limit(1)
								->execute($this->User->id);

							$arrGeonamep = deserialize($objUser->geonamep);

							if (is_array($arrGeonamep) && in_array('create', $arrGeonamep)) {
								$arrGeonames   = deserialize($objUser->areasoflaw);
								$arrGeonames[] = Input::get('id');

								$this->Database->prepare("UPDATE tl_user SET geonames=? WHERE id=?")
									->execute(serialize($arrGeonames), $this->User->id);
							}
						} // Add permissions on group level
						elseif ($this->User->groups[0] > 0) {
							$objGroup = $this->Database->prepare("SELECT geonames, geonamep FROM tl_user_group WHERE id=?")
								->limit(1)
								->execute($this->User->groups[0]);

							$arrGeonamep = deserialize($objGroup->areasoflawp);

							if (is_array($arrGeonamep) && in_array('create', $arrGeonamep)) {
								$arrGeonames   = deserialize($objGroup->areasoflaw);
								$arrGeonames[] = Input::get('id');

								$this->Database->prepare("UPDATE tl_user_group SET geonames=? WHERE id=?")
									->execute(serialize($arrGeonames), $this->User->groups[0]);
							}
						}

						// Add new element to the user object
						$root[]                 = Input::get('id');
						$this->User->geonames = $root;
					}
				}
			// No break;
			...
			case 'delete':
			case 'show':
				if (!in_array(Input::get('id'), $root) || (Input::get('act') == 'delete' && !$this->User->hasAccess('delete', 'areasoflawp'))) {
					$this->log('Not enough permissions to ' . Input::get('act') . ' geoname ID "' . Input::get('id') . '"', __METHOD__, TL_ERROR);
			...
			case 'overrideAll':
				...
				if (Input::get('act') == 'deleteAll' && !$this->User->hasAccess('delete', 'areaoflawp')) {
			...
			default:
				if (strlen(Input::get('act'))) {
					$this->log('Not enough permissions to ' . Input::get('act') . ' geoname', __METHOD__, TL_ERROR);
					$this->redirect('contao/main.php?act=error');
				}
				break;
		}
